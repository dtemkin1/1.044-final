...
from docker_python import (
    start_container,
    run_command_in_container,
    get_current_directory,
    transfer_path_by_system,
)
...
def main():
    ...
    with open(
        f"{MCDP_LIB_NAME}.mcdplib/battery_catalogues/battery_catalogue_template.yaml",
        "r",
        encoding="UTF-8",
    ) as file_stream:
        ...
        with open(
            f"{MCDP_LIB_NAME}.mcdplib/battery_catalogues/battery_catalogue_1.yaml",
            "w",
            encoding="UTF-8",
        ) as file_stream:
        ...
        for resource_element in optimistic_antichain:
            ...
            for single_resource in resource_element:
                ...
                if isinstance(single_resource, tuple):
                    print(
                        "The resource element is a Tuple (element in the product of posets)"
                    )
